...
Recast for Claude Code — MCP tools backed by the hosted Recast MCP server, plus skills for the public Reporter, Forecaster, Optimizer, and Plans APIs.
...
| Plans API | `/recast:plans-api` | Creates Plans (from scratch or from a successful optimization), edits a plan by adding a new version, creates, edits and deletes Goals, and reads Plans, versions, budgets, forecasts/counterfactuals, spend adherence, and Goal pacing |
