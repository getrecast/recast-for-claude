...
| **API skills** — code the `reporter-api` / `forecaster-api` / `optimizer-api` skills generate to call the Recast API directly | a **`RECAST_PAT`** in the environment | **Yes** |
...
`forecaster-api`, or `optimizer-api` skills — those call the Recast API directly
